...
public class BasicBSONDecoder implements BSONDecoder {
    ...
    public int decode(final byte[] bytes, final BSONCallback callback) {
        ...
        try {
            BsonWriter writer = new org.bson.BSONCallbackAdapter(new BsonWriterSettings(), callback);
            writer.pipe(reader);
    ...
    private byte[] readFully(final InputStream input) throws IOException {
        ...
        org.bson.io.Bits.readFully(input, sizeBytes);
        int size = org.bson.io.Bits.readInt(sizeBytes);
